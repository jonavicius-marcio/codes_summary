...
path_stage = "/home/marcio/projects/study/airflow/dags/data/datasets/"
...
def _load_transform():
    housing_df = pd.read_csv(
        '/home/marcio/projects/study/airflow/dags/data/datasets/usa_housing.csv')

    X = housing_df[['Avg. Area Income', 'Avg. Area House Age',
...
def choose_best_model(ti):
    ...
    r2_models = ti.xcom_pull(key='model_r2', task_ids=[
                             "train_linear_reg", "train_random_reg", "train_knn_reg"])
    index_best_model = r2_models.index(max(r2_models))

    print('Melhor modelo: {}, Score: {}'.format(
        models[index_best_model], r2_models[index_best_model]))
...
load_transform_task >> [train_linear_regression_task, train_random_regression_task, train_knn_regression_task
