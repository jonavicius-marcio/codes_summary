...
main_dir = '/home/marcio/projects/study/airflow/dags/'
...
def processing_files_csv():
    ...
    for file_name in glob.glob(main_dir + '/data/data_lake/*.json'):
        _df = pd.read_json(file_name)
        print("Concating file {}".format(file_name))
        df_people = pd.concat([df_people, _df])

    # Export data to csv
    df_people.to_csv(main_dir + "/data/data_lake/people.csv", index=False)


check_folder = FileSensor(
    # a cada 30 seg o airflow vai executar
    task_id="check_folder_export",
    poke_interval=30,
    TimeoutError=180,  # give error after 6 tentative
    filepath="/tmp/export_data", dag=dag
)
